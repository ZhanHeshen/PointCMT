...
class Pointnet2MSG(nn.Module):
    ...
    def forward(self, pointcloud, fc_only = False):
        ...
        if fc_only:
            mv_feature = pointcloud
            return self.fc_layer2(mv_feature)
        else:
            xyz, features = self._break_up_pc(pointcloud)

            features = None
            for module in self.SA_modules:
                xyz, features = module(xyz, features=features)

            features = self.fc_layer1(features.squeeze(-1))
            return self.fc_layer2(features), features
